...
(function () {
  ...
  async function initPermissionFlow() {
    ...
    /* Cooldown: 3 days (was 7). Reset if user explicitly taps the bell. */
    var lastAsked = parseInt(localStorage.getItem(PERM_KEY) || '0', 10);
    if (Date.now() - lastAsked < 3 * 24 * 60 * 60 * 1000) return;

    /* Try to get deadline info — but don't block the prompt if it fails */
    var dlInfo = null;
    try { dlInfo = await fetchNextDeadline(); } catch (_) {}

    var highlight = dlInfo
      ? dlInfo.gwName + ' — ' + dlInfo.deadlineStr
      : 'Never miss a GW deadline again';
  ...
  /* Called by the bell button to force-show the prompt regardless of cooldown */
  async function forcePrompt() {
    if (!window.Notification) return;
    if (Notification.permission === 'granted') {
      /* Already granted — show a small confirmation toast instead */
      var toast = document.createElement('div');
      toast.style.cssText = 'position:fixed;bottom:calc(76px + env(safe-area-inset-bottom,0px));left:50%;transform:translateX(-50%);background:var(--surface,#0b101e);border:1px solid var(--border-mid,rgba(255,255,255,.09));border-radius:100px;padding:10px 18px;font-size:13px;font-weight:600;color:var(--accent,#00e87a);z-index:9000;white-space:nowrap;box-shadow:0 8px 32px rgba(0,0,0,.4);pointer-events:none;transition:opacity .3s ease';
      toast.textContent = '✓ Notifications are on';
    ...
    if (Notification.permission === 'denied') {
      var toast = document.createElement('div');
      toast.style.cssText = 'position:fixed;bottom:calc(76px + env(safe-area-inset-bottom,0px));left:50%;transform:translateX(-50%);background:var(--surface,#0b101e);border:1px solid rgba(255,51,82,.2);border-radius:100px;padding:10px 18px;font-size:13px;font-weight:600;color:#ff3352;z-index:9000;white-space:nowrap;box-shadow:0 8px 32px rgba(0,0,0,.4);pointer-events:none;transition:opacity .3s ease';
      toast.textContent = '🔕 Blocked in browser settings';
      document.body.appendChild(toast);
      setTimeout(() => { toast.style.opacity='0'; setTimeout(() => toast.remove(), 300); }, 2800);
      return;
    }
    /* Default — remove cooldown and show prompt immediately */
    localStorage.removeItem(PERM_KEY);
    var dlInfo = null;
    try { dlInfo = await fetchNextDeadline(); } catch (_) {}
    var highlight = dlInfo ? dlInfo.gwName + ' — ' + dlInfo.deadlineStr : 'Never miss a GW deadline again';
    showPromptCard({
      title: 'Stay ahead of the game',
      lines: [
        '⏰ 30-min deadline reminder before each GW',
  ...
  window.KopalaNotify = {
    ...
    prompt: async function () { return forcePrompt(); },

    _promptLegacy: async function () {
