...
// MINI-LEAGUE STANDINGS (CORRECTED)
...
async function loadMostCaptained(data) {
    ...
    const captaincyPercentage = captain.selected_by_percent; // Use selected_by_percent from player element
    
    // Fallback to event percentage if player data is missing it
    const displayPercentage = captaincyPercentage || currentEvent.most_captained_percentage;
    ...
    const div = document.createElement("div");
    div.textContent = `${captain.first_name} ${captain.second_name} (${teamAbbreviation}) (£${playerPrice}m) - ${displayPercentage}%`;
