 ...
 * Version: 2.5 (Persistent Caching & Skeleton UI)
 */
...
const state = {
    ...
    playerMap: {}, 
    currentGW: 1,
    // Load image cache from local storage or start empty
...
/**
 * 1. INITIALIZATION & IMAGE HELPERS
 */
...
// Helper: Persistent Save for Image Cache
function saveImageCache() {
    localStorage.setItem('kopala_img_cache', JSON.stringify(state.imageCache));
}

// Helper: Generate Image HTML with Shimmer & Fallback
function getPlayerImageHtml(playerCode, className = "") {
    const playerKey = `p${playerCode}`;
    
    // If we already know this image fails (CORB/404), return a static white skeleton
    if (state.imageCache[playerKey] === 'failed') {
        return `<div class="${className} img-missing" style="width:110px; height:140px; background:#fff; border-radius:4px;"></div>`;
    }
    ...
    return `
        <div class="img-container ${className} skeleton-loading">
            <img src="${primaryUrl}" 
                 loading="lazy"
                 onload="handleImageSuccess(this, '${playerKey}')"
                 onerror="handleImageError(this, '${playerKey}')"
    ...
    container.innerHTML = players.map(p => `
        <div class="player-impact-card">
            <div class="shirt-wrapper">
                ${getPlayerImageHtml(state.playerMap[p.id]?.code)}
            </div>
...
function handleCompareClick() {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
    const compBtn = document.getElementById('tab-compare');
