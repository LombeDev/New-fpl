...
/* -----------------------------------------
    NAVIGATION MENU TOGGLES (NEW SECTION)
----------------------------------------- */
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hamburger && navLinks) {
        hamburger.addEventListener('click', () => {
            // Toggle the 'active' class on the navigation links list (mobile menu)
            navLinks.classList.toggle('active');
            
            // Ensure the kebab menu is closed if the main menu is opened
            if (kebabMenu) {
                kebabMenu.classList.remove('active');
            }

            // Optional: Change the icon (bars to X)
    ...
    if (kebab && kebabMenu) {
        kebab.addEventListener('click', (event) => {
            // Toggle the 'active' class on the kebab menu dropdown
            kebabMenu.classList.toggle('active');
            
            // Ensure the main navigation menu is closed if the kebab menu is opened
            if (navLinks) {
                navLinks.classList.remove('active');
                
                // Reset hamburger icon if main menu was open
                const hamburgerIcon = hamburger.querySelector('i');
                if (hamburgerIcon) {
                    hamburgerIcon.classList.remove('fa-xmark');
                    hamburgerIcon.classList.add('fa-bars');
            ...
            // Stop the click event from propagating to the document listener below
    ...
    document.addEventListener('click', (event) => {
        // Close the kebab menu if the click is outside the button or the menu
        if (kebabMenu && !kebabMenu.contains(event.target) && event.target !== kebab && !kebab.contains(event.target)) {
            kebabMenu.classList.remove('active');
        }
        
        // Close main menu when clicking a link (on mobile)
        if (navLinks && event.target.closest('.nav-links a')) {
             navLinks.classList.remove('active');
             
             // Reset hamburger icon if a link was clicked
             const hamburgerIcon = hamburger.querySelector('i');
             if (hamburgerIcon) {
                 hamburgerIcon.classList.remove('fa-xmark');
                 hamburgerIcon.classList.add('fa-bars');
