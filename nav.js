...
(function () {
  ...
  function injectMaterialSymbols() {
    const id = 'kfl-material-symbols';
    if (document.getElementById(id)) return;

    // font-display=block: holds invisible space, never flashes text fallback
    ...
    link.onload = () => {
      // Font CSS arrived — icons can now show. Use FontFace API to know
      // the exact moment the glyphs are painted, not just the CSS loaded.
      if ('fonts' in document) {
        document.fonts.load('400 22px "Material Symbols Rounded"').then(() => {
          document.body.classList.add('kfl-icons-ready');
        });
      } else {
        // Fallback: short delay then show
        setTimeout(() => document.body.classList.add('kfl-icons-ready'), 150);
      }
    };
