...
(function () {
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .kfl-dd-grid {
        ...
        gap: 8px; padding: 4px 14px 6px;
      }
      .kfl-dd-card {
        display: flex; flex-direction: column; align-items: flex-start;
        gap: 8px; padding: 13px 14px 12px; border-radius: 12px;
        background: var(--surface-2); border: 1px solid var(--border);
        text-decoration: none; color: var(--text-2);
        font-family: var(--kfl-font); font-size: 13.5px; font-weight: 600;
        cursor: pointer; -webkit-tap-highlight-color: transparent;
        touch-action: manipulation; position: relative; overflow: hidden;
        transition: background .14s ease, color .14s ease,
                    border-color .14s ease, transform .1s ease, box-shadow .14s ease;
      }
      .kfl-dd-card:active { transform: scale(0.96); }
      .kfl-dd-card.is-active {
        background: var(--active-bg); border-color: var(--accent);
        color: var(--accent); box-shadow: 0 0 0 1px var(--accent-glow) inset;
      }
      .kfl-dd-card__icon {
        width: 32px; height: 32px; border-radius: 8px;
        background: var(--surface-3);
        display: flex; align-items: center; justify-content: center;
        font-size: 14px; color: var(--text-2); flex-shrink: 0;
        transition: background .14s ease, color .14s ease;
      }
      .kfl-dd-card.is-active .kfl-dd-card__icon {
        background: var(--accent-dim); color: var(--accent);
      }
      .kfl-dd-card__label { line-height: 1; }
      .kfl-dd-card__live {
        ...
        border-radius: 100px; padding: 2px 5px 2px 4px; line-height: 1;
      }
      .kfl-dd-card__live-dot {
    ...
    const dropdown = `
      <div class="kfl-dropdown" id="kfl-dropdown" role="dialog" aria-label="Menu" aria-hidden="true" inert>
        <div class="kfl-dropdown__inner">
          ...
          <div class="kfl-dd-grid" role="navigation" aria-label="Pages">
            ${MENU_LINKS.map(l => `
              <a href="${l.href}"
                 class="kfl-dd-card${isActive(l.href) ? ' is-active' : ''}"
                 ${isActive(l.href) ? 'aria-current="page"' : ''}>
                <div class="kfl-dd-card__icon">
                  <i class="fa-solid ${l.icon}" aria-hidden="true"></i>
                </div>
                <span class="kfl-dd-card__label">${l.label}</span>
                ${l.live ? `<span class="kfl-dd-card__live" aria-label="Live now"><span class="kfl-dd-card__live-dot"></span>Live</span>` : ''}
              </a>`).join('')}
          </div>
  ...
  function syncActiveState() {
    ...
    document.querySelectorAll('.kfl-dd-card').forEach(card => {
      const a = card.getAttribute('href') === page;
      card.classList.toggle('is-active', a);
      a ? card.setAttribute('aria-current', 'page') : card.removeAttribute('aria-current');
    });
  ...
  function setupAuthGate() {
    ...
    document.querySelectorAll('.kfl-dd-card[href]').forEach(card => {
