...
(function () {
  ...
  function setupBell() {
    ...
    bell.addEventListener('click', () => {
      ...
      bell.querySelector('.fa-solid').style.animation = 'kflBellShake 0.4s ease';
      setTimeout(() => { const i = bell.querySelector('.fa-solid'); if(i) i.style.animation = ''; }, 400);
      // Trigger notify prompt
  ...
  function setupBottomNav() {
    document.querySelectorAll('.kfl-tab').forEach(item => {
      item.addEventListener('click', () => {
        if (navigator.vibrate) navigator.vibrate([6, 0, 12]); // soft double-tap feel
      });
    });
  }
