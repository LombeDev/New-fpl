...
(function () {
   ...
   *   kfl:sw-cache-hit         — SW served a response from cache
  ...
  window.KopalaPerf = {
    ...
    report() {
      ...
      if (nav) {
        console.log('%c Core Web Vitals (Navigation)', 'font-weight:bold;color:#04f5ff');
      ...
      /* kfl: marks timeline */
      console.log('%c App Marks', 'font-weight:bold;color:#04f5ff');
      ...
      /* Swap measures */
      const swaps = _perf.getEntriesByType('measure').filter(e => e.name.startsWith('kfl:'));
      if (swaps.length) {
        console.log('%c Measures', 'font-weight:bold;color:#04f5ff');
        swaps.forEach(e => {
          console.log(`  ${e.name.padEnd(30)} ${Math.round(e.duration)}ms`);
        });
      }
    ...
    /* Return a single mark's timestamp */
    get(name) {
      const e = _perf.getEntriesByName('kfl:' + name, 'mark')[0];
  ...
  window.addEventListener('load', () => {
    ...
    /* Log a brief summary on every load in dev environments */
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      ...
      navigator.serviceWorker.addEventListener('message', event => {
        ...
        if (type === 'SW_CACHE_HIT') {
          mark('kfl:sw-cache-hit');
        }
      });
    });
  }
  ...
  async function navigateTo(href, pushState = true) {
    ...
    try {
      ...
      if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
        ...
        console.log(`[PWA] Navigated to ${href} — fetch: ${fetchMs}ms, swap: ${swapMs}ms`);
      }
